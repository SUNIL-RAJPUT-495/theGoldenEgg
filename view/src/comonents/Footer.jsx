...
import { ShieldCheck, MessageCircle, MapPin, QrCode, Smartphone, X, Maximize2 } from 'lucide-react';
...
export const Footer = () => {
  ...
  return (
    <footer className="bg-[#1A2E22] text-stone-300 dark:bg-stone-950 border-t border-[#14241b] pt-16 pb-8 transition-colors duration-300 relative font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div className="space-y-4">
            ...
            {/* Storefront Full Image */}
            <div 
              onClick={() => setShowImageModal(true)}
              className="relative group overflow-hidden rounded-xl border border-white/20 bg-stone-900/60 shadow-lg my-3 cursor-pointer"
              title="Click to view full image"
            >
              <img
                src="/golden-egg-store.jpg"
                alt="Storefront"
                className="w-full h-auto rounded-xl object-contain block group-hover:scale-102 transition-transform duration-300"
              />
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="p-1.5 bg-black/60 backdrop-blur-sm rounded-full text-white block shadow-md">
      ...
      {/* Permanent Floating WhatsApp Chat Widget */}
      <a
        href="https://wa.me/917411932830?text=Hi!%20I%20am%20interested%20in%20The%20Golden%20Egg%20organic%20products."
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-50 bg-emerald-600 hover:bg-emerald-500 text-white p-3.5 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 flex items-center justify-center border-2 border-emerald-400/40 group"
        title="Chat on WhatsApp (+91 74119 32830)"
      >
        <span className="absolute -inset-1 bg-emerald-500 rounded-full animate-ping opacity-30 pointer-events-none"></span>
        <svg className="w-6 h-6 fill-current text-white relative z-10 shrink-0" viewBox="0 0 24 24">
      ...
      {showImageModal && (
        ...
        >
          <div 
            className="relative max-w-4xl max-h-[90vh] bg-stone-900/90 border border-white/20 rounded-2xl p-2 shadow-2xl overflow-hidden flex flex-col items-center animate-in fade-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setShowImageModal(false)}
              className="absolute top-4 right-4 z-10 bg-black/70 text-white p-2 rounded-full hover:bg-red-600 transition-colors shadow-lg"
              title="Close"
            >
              <X className="h-5 w-5" />
            </button>
